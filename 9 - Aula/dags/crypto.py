...
def transform_data(**kwargs):
    ...
    ftx = []
    bybit = []
    binance_ohlcv_data, ftx_ohlcv_data, fetch_bybit_ohlcv = ti.xcom_pull(
        key=None, task_ids=['fetch_binance_ohlcv', 'fetch_ftx_ohlcv', 'fetch+bybit_ohlcv'])
...
def create_text_file(**kwargs):
    ...
    abs_path_binance = os.path.abspath(file_ftx)
...
def load_data(**kwargs):
    ...
    for record in binance_data:
        sql = "INSERT INTO close_binance(close, ts) VALUES('{}', '{}')".format(
            record['close'], record['ts'])
        cursor.execute(sql)
        connection.commit()

    for record in ftx_data:
        sql = "INSERT INTO close_ftx(close, ts) VALUES('{}', '{}')".format(
            record['close'], record['ts'])
        cursor.execute(sql)
        connection.commit()

    for record in bybit_data:
        sql = "INSERT INTO close_bybit(close, ts) VALUES('{}', '{}')".format(
            record['close'], record['ts'])
        cursor.execute(sql)
        connection.commit()
...
def generate_pdf_reports(**kwargs):
    ...
    binance_data = close_data[0]['binance']
    ftx_data = close_data = close_data[0]['ftx']
    bybit_data = close_data[0]['bybit']

    initial_volume_record_binance = binance_data[0]
    final_volume_record_binance = binance_data(len(binance_data) - 1)

    binance_change_percent = calculate_percentage(
        initial_volume_record_binance['volume'], final_volume_record_binance['volume'])

    pdf_path = create_pdf(
        binance_data, binance_change_percent, 'binance_graph.png')
    return pdf_path
...
with DAG("crypto_analysis", catchup=False, default_args=default_args, schedule_interval='*/2 * * * *') as dag:

    fetch_binance_ohlcv = PythonOperator(
        task_id="fetch_binance_ohlcv", python_callable=fetch_binance_ohlcv)

    fetch_ftx_ohlcv = PythonOperator(
        task_id='fetch_ftx_ohlcv', python_callable=fetch_ftx_ohlcv)

    fetch_bybit_ohlcv = PythonOperator(
        task_id='fetch_bybit_ohlcv', python_callable=fetch_bybit_ohlcv)

    transform_data = PythonOperator(
        task_id='transform_data', python_callable=transform_data)

    create_text_file = PythonOperator(
        task_id='create_text_file', python_callable=create_text_file)

    load_data = PythonOperator(
        task_id='load_data', python_callable=load_data)

    generate_pdf_reports = PythonOperator(
        task_id='generate_pdf_reports', python_callable=generate_pdf_reports)

    ftp_file = PythonOperator(task_id='ftp_file', python_callable=ftp_file)

    ftp_pdf_file = PythonOperator(
        task_id='ftp_pdf_file', python_callable=ftp_pdf_file)
